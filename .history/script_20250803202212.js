...
async function init() {
    ...
    const globalRaw = await d3.csv("global_internet.csv");
const worldRow = globalRaw.find(d => d["Country Name"] === "World");

const globalData = Object.keys(worldRow)
    .filter(key => /^\d{4}$/.test(key))  // keep only year columns
    .map(year => ({
        Year: +year,
        InternetPenetration: +worldRow[year] || 0
    }));
