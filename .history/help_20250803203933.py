...
indicator_filter = "Individuals using the Internet (% of population)"
...
with open(input_file, newline='', encoding='utf-8') as infile, \
    ...
    # Skip metadata lines (like "Data Source", "Last Updated Date")
    while True:
        header = next(reader)
        if header[0] == "Country Name":
            break

    years = header[4:]  # Start from 5th column onwards (i.e., 1960...)
    ...
    for row in reader:
        ...
        # Only include life expectancy rows
        if indicator != indicator_filter:
            continue
