...
async function init() {
    ...
    const rawText = await d3.text("global_internet.csv");

// Split into rows
const rows = d3.csvParseRows(rawText);

// Slice off the first 2 metadata rows
...
// Find the "World" row
const worldRow = data.find(d => d[0] === "World");

// Map year values
const globalData = headers
  .map((key, i) => {
    if (/^\d{4}$/.test(key)) {
      return {
        Year: +key,
        InternetPenetration: +worldRow[i] || 0
      };
    }
    return null;
  })
  .filter(d => d !== null);Penetration: +worldRow[year] || 0
    }));
